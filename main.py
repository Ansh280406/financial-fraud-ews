...
async def verify_otp(data: OTPVerification):
    ...
    # 2. Get the specific random OTP for this session
...
async def predict_fraud(data: LoginAttempt):
    ...
    # --- GENERATE RANDOM OTP (Simulated Send) ---
    ...
    if uid not in USER_DB:
        ...
        ACTIVITY_LOGS.insert(0, {
            ...
            "risk": "0%", "action": "✅ New Profile - OTP Generated", "status": "safe"
        })
        return {"final_risk_score": 0.0, "security_action": "✅ OTP Required", "detector_scores": {}}
    ...
    if "BLOCK" in action_text: 
        status_color = "danger"
        otp_display = "BLOCKED" # Don't show valid OTP if blocked
        OTP_CACHE.pop(uid, None) # Invalidate OTP

    elif "Email" in action_text or "FLAG" in action_text:
    ...
    ACTIVITY_LOGS.insert(0, {
        ...
        "user": uid, "location": f"{data.latitude:.2f}, {data.longitude:.2f}",
        "otp": otp_display, 
        "risk": f"{result.get('final_risk_score', 0)*100:.0f}%",
        "action": action_text, 
        "status": status_color
    })
